...
pub fn find_matches(directory: &Arc<Mutex<Directory>>, regex: Regex) -> (Vec<String>, Vec<File>) {
    execute(directory, regex)
}

//----------- private -------------//

fn execute(directory: &Arc<Mutex<Directory>>, regex: Regex) -> (Vec<String>, Vec<File>) {
    ...
    fetch_matches(directory.clone(), regex.clone(), matches_queue.clone(), file_matches_queue.clone(), current_concurrency.clone(), concurrency_limit.clone());
    ...
    (merged_matches, file_merged_matches)
}


fn fetch_matches(directory: Arc<Mutex<Directory>>, regex: Regex, matches_queue: Arc<SegQueue<Vec<String>>>, file_matches_queue: Arc<SegQueue<Vec<File>>>, current_concurrency: Arc<AtomicUsize>, concurrency_limit: Arc<AtomicUsize>) {
    ...
    if is_match(&locked_directory.path, &regex) {
        ...
        for sub_directory in locked_directory.sub_directories.clone() {
            if max_concurrency_reached(current_concurrency.clone(), concurrency_limit.clone()) {
                fetch_matches(Arc::new(Mutex::new(sub_directory)), regex.clone(), matches_queue.clone(), file_matches_queue.clone(), current_concurrency.clone(), concurrency_limit.clone());
            } else {
                ...
                thread::spawn(move || {
                    ...
                    fetch_matches(Arc::new(Mutex::new(sub_directory)), local_regex, local_matches_queue, local_file_matches_queue, local_current_concurrency.clone(), local_concurrency_limit);
